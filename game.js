...
// Dodaj funkcję do rysowania ekranu pauzy
function drawPauseScreen() {
    // Zatrzymaj wszystkie dźwięki
    if (audio.gameMusic.currentTime > 0 && !audio.gameMusic.paused) {
        audio.gameMusic.pause();
    }
    
    // Narysuj semi-transparentną nakładkę
    ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
    ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
    
    // Narysuj tekst "PAUSED"
    ...
    ctx.fillText('Press M to return to menu', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 60);
}
    ...
    switch (e.key) {
        ...
        case 'Escape':
            if (gameState.running) {
                ...
                // Wznów muzykę gdy gra jest odpauzowana
                if (!gameState.paused && audio.gameMusic.currentTime > 0) {
        ...
        case 'M':
            if (gameState.running && gameState.paused) {
                ...
                // Zatrzymaj muzykę gry i włącz muzykę menu
                audio.gameMusic.pause();
                audio.gameMusic.currentTime = 0;
